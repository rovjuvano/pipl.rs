...
fn add_factorial(
    ...
    let (t1, t2, t3) = (
        &pipl.new_name(N::Str("t1")),
        &pipl.new_name(N::Str("t2")),
        &pipl.new_name(N::Str("t3")),
    );
    ...
    let c = builder
        .read(&fact).names(&[x, out]).repeat()
        .send(greater_than).restrict(&[gt2, le2]).names(&[x, two, gt2, le2])
        .choice();
    c   .read(le2)
        .send(out).names(&[x]);
    let p = c
        .read(gt2)
        .parallel().restrict(&[t1, t2, t3]);
    p   .send(subtract).names(&[x, one, t1]);
    p   .read(t1).names(&[x_minus_1])
        .send(&fact).names(&[x_minus_1, t2]);
    p   .read(t2).names(&[factorial_x_minus_1])
        .send(multiply).names(&[x, factorial_x_minus_1, t3]);
    p   .read(t3).names(&[result])
...
fn main() {
    ...
    for arg in env::args().skip(1) {
        let x = &pipl.new_name(N::Usize(usize::from_str_radix(&arg, 10).unwrap()));
